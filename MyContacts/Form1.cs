    ...
    public partial class Form1 : Form
    {
        ...
        private void btnDelete_Click(object sender, EventArgs e)
        {
            if (dgContacts.CurrentRow != null)
            {
                ...
                if(MessageBox.Show($"آیا از حذف این {fullName} مطمعن هستید؟","توجه",MessageBoxButtons.YesNo) == DialogResult.Yes)
                {
        ...
        {
            dgContacts.DataSource = repository.Serach(txtSearch.Text);

        }

        private void addState_Click(object sender, EventArgs e)
        {
            stateFrom state = new stateFrom();
            state.ShowDialog();
        }
    }
}
